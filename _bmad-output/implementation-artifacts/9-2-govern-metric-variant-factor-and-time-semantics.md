...
- [ ] 扩展 ontology registry 表结构，纳入治理对象骨架（AC: 1, 2, 3, 4）
  - [ ] 在 `9.1` 的最小 registry 基础上补充至少以下表或等价结构：
    - [ ] `platform.ontology_metric_variants`
    - [ ] `platform.ontology_time_semantics`
    - [ ] `platform.ontology_causality_edges`
    - [ ] `platform.ontology_evidence_type_definitions`
  - [ ] 为 `MetricDefinition / MetricVariantDefinition / TimeSemanticDefinition / FactorDefinition / CausalityEdgeDefinition / EvidenceTypeDefinition` 建立稳定业务键、版本引用和生命周期状态。
  - [ ] 保持 `platform` schema 为 canonical source；不要把运行时 catalog 直接升级成事实源。

- [ ] 把收费类双口径与时间语义收进正式治理模型（AC: 1, 2, 3）
  - [ ] 从当前真实收费语义实现中抽取并治理化这些对象：
    - [ ] `项目口径`
    - [ ] `尾欠口径`
    - [ ] `receivable-accounting-period`
    - [ ] `billing-cycle-end-date`
    - [ ] `payment-date`
  - [ ] 明确这些定义与现有 Cube 语义查询适配层之间的映射关系。
  - [ ] 不在本 story 中重写全部 Cube adapter，只需要把治理定义和运行时映射边界站稳。

- [ ] 把候选因素、因果边与证据类型拉入正式治理对象（AC: 1, 2, 4）
  - [ ] 将当前真实候选因素扩展中已经稳定存在的核心因素定义纳入 registry。
  - [ ] 为 Neo4j 因果关系查询建立 `CausalityEdgeDefinition` 语义骨架，明确“图谱关系”与“允许作为归因路径的治理关系”不是一回事。
  - [ ] 为 conclusion renderer 与 execution result block 定义 `EvidenceTypeDefinition`，至少覆盖：
    - [ ] 表格证据
    - [ ] 图谱证据
    - [ ] ERP 事实证据
    - [ ] 模型摘要证据

- [ ] 建立治理化读取路径并限制未审批定义进入运行时（AC: 1, 4）
  - [ ] 在 `src/application/ontology/` 下补充读取 use cases，使运行时可以按版本和生命周期读取上述定义。
  - [ ] 为默认运行时路径明确“仅 `approved` 定义可用”的规则。
  - [ ] 若当前旧代码仍需要兼容硬编码 catalog，必须明确其为 transitional path，并提供替换点；不要把兼容逻辑伪装成正式方案。

- [ ] 补齐 story 级验证（AC: 1, 2, 3, 4）
  - [ ] 验证双口径与时间语义已进入 registry，并可按版本读取。
  - [ ] 验证 factor / causality edge / evidence type 可被正式读取。
  - [ ] 验证未 `approved` 定义不会进入默认运行时选择。
  - [ ] 验证旧运行时映射层至少可消费治理后的 canonical definitions。
...
- 必须遵循 [ontology-governance-architecture.md](/Users/delldi/work-code/open-code/ontology-agent/_bmad-output/planning-artifacts/ontology-governance-architecture.md) 中“最先收敛四类对象”的判断：
