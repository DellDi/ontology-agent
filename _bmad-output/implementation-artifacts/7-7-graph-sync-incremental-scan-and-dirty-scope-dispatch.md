...
- [ ] 建立增量 cursor 模型与持久化表（AC: 1, 3）
  - [ ] 为 `platform.graph_sync_cursors` 增加 Drizzle schema 与迁移，至少覆盖 `source_name`、`cursor_time`、`cursor_pk`、`last_run_id`、`updated_at`。
  - [ ] 为每个 source 明确初始命名与候选字段，至少包括 `erp.organizations / projects / owners / charge_items / receivables / payments / service_orders`。
  - [ ] 统一采用“时间戳 + 主键”复合游标语义；若某 source 只能退化为主键扫描，必须记录在 run error/detail 或等价诊断上下文中。
- [ ] 建立 dirty scope 队列表与语义去重（AC: 1, 2, 3）
  - [ ] 为 `platform.graph_sync_dirty_scopes` 增加 Drizzle schema 与迁移，至少覆盖 `id`、`scope_type`、`scope_key`、`reason`、`source_name`、`source_pk`、`first_detected_at`、`last_detected_at`、`status`、`attempt_count`、`last_run_id`。
  - [ ] 保证 pending 状态下的 `(scope_type, scope_key)` 语义去重，避免同一组织因短时间多条变更无限堆积重复任务。
  - [ ] 明确状态流转最少包含 `pending / processing / completed / failed`。
- [ ] 实现 watermark 扫描与组织范围提取（AC: 1）
  - [ ] 新增 graph sync incremental scanner use case，按 source 读取 cursor、扫描变更并提取受影响 `organizationId`。
  - [ ] 组织范围提取遵循 [graph-sync-operating-model.md](/Users/delldi/work-code/open-code/ontology-agent/docs/data-contracts/graph-sync-operating-model.md) 中定义的 source 规则，特别是：
  ...
  - [ ] 对扫描结果写入 dirty scope，而不是在 scanner 阶段直接触发 Neo4j rebuild。
- [ ] 实现 dirty scope dispatcher 与 cursor 推进边界（AC: 2, 3）
  - [ ] dispatcher 必须消费 pending dirty scope，并调用 `7.6` 提供的 `org-rebuild` 正式执行单元。
  - [ ] cursor 推进必须发生在成功派发并完成对应组织重建之后，不能在“只扫描到变更”后立即前移。
  - [ ] 若某组织 rebuild 失败，dirty scope 必须保留失败状态与重试信息，cursor 不得跳过该 source 的未完成处理。
- [ ] 补齐增量扫描与派发测试（AC: 1, 2, 3）
  - [ ] 验证按复合游标只扫描新变更，避免同一窗口重复或漏扫。
  - [ ] 验证多条变更命中同一组织时只生成一个 pending dirty scope。
  - [ ] 验证 dirty scope 成功后才推进 cursor，失败时 cursor 保持不变。
  - [ ] 验证 dispatcher 最终调用的是组织级 `org-rebuild`，而不是单记录 patch。
...
- 必须遵循 [graph-sync-operating-model.md](/Users/delldi/work-code/open-code/ontology-agent/docs/data-contracts/graph-sync-operating-model.md) 的路线：增量阶段优先按 `organizationId` 重建，避免逐条 CDC patch 图边。
...
- [Story 7.6](/Users/delldi/work-code/open-code/ontology-agent/_bmad-output/implementation-artifacts/7-6-graph-sync-run-metadata-and-org-rebuild.md) 应先提供正式 `org-rebuild` 单元与 graph sync run metadata；`7.7` 必须建立在它之上，而不是重复实现 run 逻辑。
- [Story 4.5](/Users/delldi/work-code/open-code/ontology-agent/_bmad-output/implementation-artifacts/4-5-neo4j-graph-adapter-and-sync-baseline.md) 已站稳 baseline builder 与 controlled sync path，所以增量派发必须复用既有 graph sync use case。
- [graph-sync-operating-model.md](/Users/delldi/work-code/open-code/ontology-agent/docs/data-contracts/graph-sync-operating-model.md) 已明确 source 提取规则、cursor 语义和 dirty scope 路线，`7.7` 的实现不应偏离该文档。
- 最近提交 [06bf22f](/Users/delldi/work-code/open-code/ontology-agent/.git) 说明 graph sync 和真实分析执行已经进入真实环境链路；因此 `7.7` 要按“生产可靠性”标准做，而不是临时脚本。
...
- [project-context.md](/Users/delldi/work-code/open-code/ontology-agent/_bmad-output/project-context.md) 仍要求：
