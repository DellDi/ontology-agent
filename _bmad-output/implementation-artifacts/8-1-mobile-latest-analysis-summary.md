...
4. 移动端摘要必须来自与 PC 同源的 render schema 投影，而不是单独维护一套不兼容 DTO。

## Tasks / Subtasks

- [ ] 建立移动端最近分析 read model（AC: 1, 2, 3, 4）
  - [ ] 定义“最近一次分析”的服务端判定规则。
...
- 移动端摘要字段应来自稳定持久化结果和同源 render schema 投影，而不是页面临时拼装。
...
- 5.4 和 6.4 的结果与历史持久化是移动端摘要的主要数据基础。
- 7.1 的服务端权限校验是移动端能力安全上线前置。

### References

- [Source: _bmad-output/planning-artifacts/epics.md#Story 8.1: 移动端查看最近分析摘要与同源结果投影]
...
- _bmad-output/implementation-artifacts/8-1-mobile-latest-analysis-summary.md
