...
### Process Board Side Sheet

流程看板必须是可隐藏的 `side sheet`，而不是永久占据主画布的卡片堆叠：

- 默认态：收起或半展开，主画布优先展示阶段结果与结论叙事。
- 专家态：展开后显示步骤轨迹、工具态、推理摘要、assumptions、阻断原因。
- 切换态：收起/展开不应中断执行流，也不应丢失当前阅读锚点。
- 恢复态：刷新或重进会话后，side sheet 的展开状态可恢复。
...
- `process-board`
  side sheet 容器，承载步骤轨迹、工具态、推理摘要和 assumptions。
- `markdown`
...
- `reasoning-summary`
  表达当前阶段为何推进、为何暂停、为何切换策略。
- `assumption-card`
  表达在 non-blocking 缺省下继续执行时采用的显式假设与风险提示。
...
负责把本增补文档中的 `Process Board Side Sheet` 落成可隐藏、可恢复、可诊断的专家过程看板。
