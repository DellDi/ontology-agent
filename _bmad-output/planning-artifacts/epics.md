...
FR2: Epic 3, Epic 9 - 识别分析意图并建立统一业务语义  
FR3: Epic 3 - 抽取分析上下文  
...
**Given** 会话已完成基础 intent/context 提取且不存在高风险阻断项  
**When** 用户刚进入会话主界面  
**Then** 系统应默认自动提交首轮执行任务  
**And** 不应要求用户先手工补齐所有条件才允许执行

**Given** 存在普通缺省字段（如比较基线或次要约束）  
**When** 系统判断不属于高风险冲突  
**Then** 系统应以显式 assumptions 继续执行  
**And** 在结果与过程看板中清楚披露本轮假设
...
**Given** 用户处于深度分析阅读场景  
**When** 系统持续回传步骤、工具和阶段状态  
**Then** 会话界面应提供可隐藏的侧滑流程看板（process board / side sheet）  
**And** 用户在收起看板时仍可保持主结果叙事流阅读不被打断
...
**Given** 用户未补充任何增量条件  
**When** 系统完成首轮分析  
**Then** 系统仍应提供可执行结论和后续建议  
**And** 不应以“未补充条件”为由阻断默认执行路径
...
**Given** 用户未触发纠偏  
**When** 系统推进默认分析流程  
**Then** 应沿当前主链持续执行并回传阶段结果  
**And** 不应等待“先纠偏再执行”的交互前置
...
- Epic 7 主线按 `7.1 -> 7.2 -> 7.3/7.4 -> 7.5` 执行。
- `7.6 ~ 7.8` 属于 graph sync 运行化补强，已作为独立支线先行落地，不改变 Epic 7 主线优先级。
- 后续 sprint 执行与状态解读应以该顺序为准，避免把“运行化已完成”误读成“治理主线已完成”。
...
**And** 除 `web / worker / Postgres / Redis / Cube / Neo4j` 外，还应明确 `graph sync job runner / scheduler` 的运行边界
...
**Given** 平台已在试点环境运行
**When** 运维团队查看系统监控
**Then** 应能看到基础服务健康状态、分析任务状态、graph sync run 状态、dirty scope 积压和关键错误信息
...
So that 我可以复用已同步的 ERP 用户目录与组织权限完成登录和范围投影，而不需要再建设一套新的组织、用户、角色权限系统。
...
**When** 用户在登录页输入账号和密码  
**Then** 系统应先基于 `dw_datacenter_system_user.user_account` 解析到唯一用户，再调用远端加密接口对输入密码执行加密，并将返回的 `result` 与 `dw_datacenter_system_user.user_password` 比较  
**And** 成功后创建服务端会话并带入该用户的组织与项目范围  
**And** 不再要求用户手动输入组织、项目、区域或角色信息

**Given** 登录 URL 中包含账号参数，且内部桥接入口已启用  
**When** 系统能解析到唯一有效用户  
**Then** 系统可直接创建服务端会话并跳转到受保护页面  
**And** 该桥接流程不要求再次输入密码  
**And** 必须明确标记为内部试点或非生产登录路径

**Given** 账号不存在、命中多条、用户已停用、远端加密接口失败，或密码校验失败  
...
**And** 平台只保留最小服务端会话，不新增本地组织/用户/RBAC 权威
...
> **Status (2026-04-17)**：`merged-into-10.5`。  
> 本 Epic 的 3 个 story 已归并到 `Story 10.5` 的内部 Tasks，避免移动端投影出现两条并行实现路径。详见
> `_bmad-output/planning-artifacts/sprint-change-proposal-2026-04-17-b.md`。
> 产品能力目标（移动端独立 surface）不变，仅不再作为独立 sprint 轨道推进。
>
> - 原 8.1 → 10.5 Task: 移动端最近分析摘要投影
> - 原 8.2 → 10.5 Task: 移动端关键证据视图
> - 原 8.3 → 10.5 Task: 移动端轻量追问附着到原会话，不生成独立协议
...
### Execution Order (2026-04-17)

- `9.1` ✅ done → `9.2 / 9.3` 强制在 `Story 10.1` 开工之前合入（grounding 契约冻结）。
...
- `9.6` 与 `Story 10.5` 有耦合（mobile projection 也要版本绑定），可并行或紧邻 10.5 执行。
...
`Story 10.6` 已于 2026-04-17 提前完成并 `done`（无 spec 插入开发），为保持运行时抽象一致性，Epic 10 剩余 story 按以下顺序推进：

`10.1 → 10.2 → 10.3 → 10.4 → 10.5 → 10.7 (alignment sweep)`

- `10.1` 落地时必须保留 `10.6` 已建立的客户端行为语义（见 10.1 AC 增补条款）。
- `10.2` 的 renderer registry 将承接 `10.6` 在 UX addendum 预留的 `process-board` / `reasoning-summary` / `assumption-card` 三个 part 的正式实现。
- `10.7` 作为 Epic 10 收口检查点，回归 10.6 既有能力并关闭 `deferred-work.md` 中 W1 / W2 项。
...
**Given** `Story 10.6` 已提前落地侧滑流程看板、自动执行闸门与 assumption 投影
**When** 10.1 runtime adapter 接管 `AnalysisExecutionLiveShell` 的事件合并
...
### Story 10.6: 可隐藏的执行流程看板与专家模式

As a 深度分析用户,
I want 在会话页使用可隐藏的侧滑流程看板查看步骤、工具、状态和推理摘要,
So that 我既能专注主结果叙事流，也能在需要时进入专家级过程审阅。

**关联需求：** FR7, FR13, FR17, NFR2

**Acceptance Criteria:**

**Given** 分析任务处于流式执行阶段  
**When** 用户查看会话主界面  
**Then** 系统应在 side sheet / process board 中展示步骤轨迹、工具态、状态与推理摘要  
**And** 这些信息不应与主结论流混成同一阅读层级

**Given** 用户切换“展开看板 / 收起看板”  
**When** 会话持续回传执行事件  
**Then** 看板状态应可持久化并在刷新后恢复  
**And** 收起看板不应导致执行状态丢失或中断

**Given** 流程看板中存在高风险阻断或失败事件  
**When** 用户在看板中查看详情  
**Then** 系统应提供明确原因与下一步动作建议  
**And** 不得以静默失败替代可诊断反馈
...
So that `Story 10.6` 已提前落地的交互主链在 runtime 抽象上线后仍然满足 AC1~AC5，且 `reasoning-summary` / `assumption-card` / `process-board` 三个 UX part 已经通过正式 renderer-registry 消费。

**关联需求：** FR7, FR8, FR17, NFR2

**Acceptance Criteria:**

**Given** `10.1 / 10.2 / 10.3 / 10.4 / 10.5` 已全部合入
**When** 回归 `Story 10.6` 的 Review Findings（P1~P12 + D1~D5）
**Then** 这些 findings 必须已映射到对应的 runtime / renderer 正式实现
**And** 不再有"临时实现 / 等待 10.x 落地"标签

**Given** `Story 10.6` 已有测试与新增的 runtime / renderer 测试
**When** 执行 `tests/story-10-6-grounded-planning.test.mts` 与 `tests/story-10-1-*` / `tests/story-10-2-*`
**Then** 全部通过
**And** 关键 UI 行为（默认收起 side sheet / Esc 关闭 / 自动执行 scope 去重 / assumption 投影）由 story-based integration test 覆盖

**Given** `deferred-work.md` 中 W1（`parseProgressText`）与 W2（`AnalysisPlan._executionAssumptions` 下划线约定）作为 Story 10.6 review 遗留
**When** Alignment Sweep 结束
**Then** W1 必须已在 `Story 10.2 renderer-registry` 内正式收口
**And** W2 必须作出明确决策（去下划线改为正式 domain 字段 OR 将"下划线=运行时只读标注"写入 domain 规范）并已落地
