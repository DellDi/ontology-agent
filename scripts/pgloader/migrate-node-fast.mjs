...
// 从环境变量获取配置：
// mysql -u b9d73f86-2233-492d-bfe3-ee7d92e78f1f -gHK1csA1qhRv8inF -h jms.new-see.com -P 33061
const MYSQL_URL =
  process.env.MYSQL_URL ||
  'mysql://b9d73f86-2233-492d-bfe3-ee7d92e78f1f:gHK1csA1qhRv8inF@jms.new-see.com:33061/newsee-datacenter';
const DATABASE_URL =
...
function toSnakeCase(str, tableName) {
  const tableMapping = tableName ? columnMappings[tableName] : undefined;

  if (tableMapping && Object.hasOwn(tableMapping, str)) {
    return tableMapping[str];
  }
...
async function migrateTable(tableName, mysqlConn, pgClient) {
  ...
  const validColumns = columns.filter((column) => {
    const pgName = toSnakeCase(column.Field, tableName);

    return pgName !== null;
  });
  const validColumnNames = validColumns.map((column) => column.Field);
  const validPgColumnNames = validColumnNames.map((column) =>
    toSnakeCase(column, tableName),
  );
  ...
  for (let offset = 0; offset < count; offset += BATCH_SIZE) {
    const [rows] = await mysqlConn.query('SELECT * FROM ?? LIMIT ? OFFSET ?', [
      tableName,
      BATCH_SIZE,
      offset,
    ]);
  ...
  const rps = (count / Number.parseFloat(duration)).toFixed(0);
...
async function main() {
  ...
  const mysqlConn = await mysql.createConnection({
    ...
    database:
      process.env.PROJECTION_DB || mysqlUrl.pathname.replace(/^\/+/, ''),
  });
  ...
  const avgRps = (totalRows / Number.parseFloat(totalDuration)).toFixed(0);
  ...
  results.forEach((result) => {
    ...
    const speed = result.rps ? ` (${result.rps} 行/秒)` : '';
    console.log(
      `  ${status} ${result.tableName}: ${result.count} 行 (${result.duration}s)${speed}`,
    );
  });
}
