...
- `easyv/internal/adapter/out/postgres/EasyVPostgresFactAdapter` 以 READ ONLY、allowlist schema 和聚合结果读取四类 source；部分 duration 缺失会显式披露 timed/eligible 覆盖率，只有全量缺失才返回 duration missing。
...
- `backend-java/src/main/java/com/dip3/ontologyagent/easyv/internal/adapter/out/postgres/EasyVPostgresFactAdapter.java`：EasyV 开发源只读聚合 adapter 与 freshness/耗时边界；
