...
本文档是 [graph-sync-baseline.md]({project-root}/docs/data-contracts/graph-sync-baseline.md) 的运行时补充。前者定义“同步什么”，本文档定义“怎么持续同步、怎么运营交付”。
...
- 组织级 baseline 读取与建图：
  - [graph sync use cases]({project-root}/src/application/graph-sync/use-cases.ts)
  - [PG ERP read repository]({project-root}/src/infrastructure/erp/postgres-erp-read-repository.ts)
- Neo4j 幂等写入：
  - [graph adapter]({project-root}/src/infrastructure/neo4j/neo4j-graph-adapter.ts)
  - [graph sync cypher]({project-root}/src/infrastructure/sync/neo4j-graph-sync.ts)
- 基线同步入口：
  - [sync-neo4j-baseline.mts]({project-root}/scripts/sync-neo4j-baseline.mts)
...
- [graph:sync:neo4j]({project-root}/package.json)
