...
test('Neo4j adapter 仅服务端可用，并使用官方 driver 执行读写查询', async () => {
  ...
  assert.match(adapter, /getSharedDriver/);
  assert.match(adapter, /closeSharedNeo4jDriver/);
  assert.match(adapter, /executeQuery|session\(/);
  assert.match(adapter, /MATCH/);
  assert.match(adapter, /buildNeo4jNodeSyncCypher/);
  assert.match(sync, /MERGE/);
  assert.match(adapter, /has-receivable|has-payment|has-service-order/);
  assert.doesNotMatch(
    adapter,
    /finally\s*{[^}]*driver\.close\(/s,
    '默认查询路径不应每次关闭 Neo4j driver，否则会绕过 driver 连接池',
  );
});
