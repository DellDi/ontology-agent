...
test('工作台流式面板与结论面板不得继续复制页面级 block.type 分支渲染', async () => {
  ...
  const conclusionPanel = await readFile(
    resolve(
      projectRoot,
      'src/app/(workspace)/workspace/analysis/[sessionId]/_components/analysis-static-assistant-props.ts',
    ),
    'utf8',
  );
  ...
  assert.match(conclusionPanel, /renderAnalysisInteractionPart/);
