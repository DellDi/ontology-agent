...
test('归因类问题展示候选影响因素与可解释依据', async () => {
  ...
  assert.match(
    html,
    /这些因素不是最终结论|图谱候选因素暂不可用/,
  );
  assert.match(
    html,
    /图谱关系与当前指标的相关依据|与当前指标或实体的相关依据/,
  );
  assert.match(html, /候选方向/);
  assert.match(html, /关系/);
  assert.match(html, /来源/);
  assert.match(html, /erp-derived|erp-master-data|governed-rule/);
});
...
test('简单查询类问题跳过候选因素扩展，不强行展示因素列表', async () => {
  ...
  assert.doesNotMatch(html, /服务响应时长/);
  assert.doesNotMatch(html, /governed-rule/);
});
