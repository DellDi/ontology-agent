...
test('归因结果模型会稳定生成排序原因、证据摘要和表格类结果块', async () => {
  const result = await runTsSnippet(`
    ...
    console.log(JSON.stringify({
      ...
      firstCauseConfidence: readModel.causes[0]?.confidence ?? null,
  ...
  assert.equal(result.firstCauseConfidence, null);
...
test('分析会话页会展示排序后的原因列表、关键证据和表格类结论块', async () => {
  ...
  assert.match(html, /证据评分待补充/);
  assert.doesNotMatch(html, /置信度\s*\d+%/);
  assert.match(html, /检查指标变化|验证候选因素/);
});

test('流式事件推进后会生成实时归因结论', async () => {
  const result = await runTsSnippet(`
    import displayModule from './src/app/(workspace)/workspace/analysis/[sessionId]/analysis-execution-display.ts';

    const { mergeExecutionEvents, buildLiveConclusionReadModel } = displayModule;

    const nextEvents = mergeExecutionEvents(
      [],
      {
        id: 'evt-live-1',
    ...
    const conclusion = buildLiveConclusionReadModel({
      events: nextEvents,
      fallbackReadModel: null,
    });

    console.log(JSON.stringify({
      eventCount: nextEvents.length,
      causeCount: conclusion?.causes.length ?? 0,
      firstCauseTitle: conclusion?.causes[0]?.title ?? null,
    }));
  `);
