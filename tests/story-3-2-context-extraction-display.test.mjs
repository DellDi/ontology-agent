...
test('真实业务问法里项目名位于“项目”之前时，仍能抽取到正确实体和指标', async () => {
  const cookie = await login({
    employeeId: 'ctx-u-4',
    displayName: '上下文测试员4',
  });
  ...
  const { html } = await createSessionAndGetPage({
    cookie,
    questionText:
      '近三个月丰和园小区项目的收缴率和欠费压力为什么异常？请结合应收、实收与欠费分析。',
  });

  assert.match(html, /丰和园小区项目/);
  assert.match(html, /收缴率/);
  assert.doesNotMatch(html, /项目 的收缴率和欠费压力为什么异常/);
  assert.doesNotMatch(html, /待补充目标指标/);
});
