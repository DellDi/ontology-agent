...
test('ERP 领域模型与 scope 过滤规则显式存在，并支持组织 / 项目 / 区域裁剪', async () => {
...
test('受限项目 scope 会拒绝缺少 projectId 的事实记录，但不会再要求 areaId', async () => {
  const output = await runTsx(`
    ...
    const noAreaStillAllowed = canAccessErpScope(scope, {
      organizationId: 'org-parent',
    ...
    console.log(JSON.stringify({
      missingProject,
      missingArea: noAreaStillAllowed,
      completeRecord,
    }));
  `);
