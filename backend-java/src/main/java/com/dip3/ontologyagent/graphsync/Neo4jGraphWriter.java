...
public final class Neo4jGraphWriter implements GraphWriter {
    static final String FENCE_CONSTRAINT = """
            create constraint graph_sync_fence_scope if not exists
            for (f:GraphSyncFence) require f.scope_org_id is unique
            """;
    ...
    static final String NODE_MERGE = """
            ...
            merge (n:GraphNode {scope_org_id:node.organizationId,kind:node.kind,id:node.id})
            set n.label=node.label,n.scope_org_id=node.organizationId,n.last_seen_run_id=node.runId
            """;
    static final String EDGE_MERGE = """
            unwind $edges as edge
            match (from:GraphNode {scope_org_id:edge.organizationId,kind:edge.fromKind,id:edge.fromId})
            match (to:GraphNode {scope_org_id:edge.organizationId,kind:edge.toKind,id:edge.toId})
            ...
            set r.direction=edge.direction,r.source=edge.source,r.explanation=edge.explanation,
                r.scope_org_id=edge.organizationId,r.last_seen_run_id=edge.runId
            """;
    static final String EDGE_CLEANUP = """
            match ()-[r:GRAPH_EDGE]->()
            where r.scope_org_id=$organizationId and r.last_seen_run_id<>$runId delete r
            """;
    static final String NODE_CLEANUP = """
            match (n:GraphNode {scope_org_id:$organizationId})
            where n.last_seen_run_id<>$runId
              and not exists { match (n)-[r]-() where r.scope_org_id=$organizationId }
            delete n
            """;
    ...
    @Override
    public WriteResult replaceOrganization(String organizationId, String runId, long fencingToken,
                                           GraphBatch batch) {
        validateBatch(organizationId, runId, batch);
        try {
            ...
            try (var session = driver.session(sessionConfig)) {
                session.executeWrite(tx -> {
                    ...
                    Map<String, Object> cleanup = Map.of("organizationId", organizationId, "runId", runId);
    ...
    private static void validateBatch(String organizationId, String runId, GraphBatch batch) {
        for (Map<String, Object> item : java.util.stream.Stream.concat(batch.nodes().stream(), batch.edges().stream())
                .toList()) {
            if (!organizationId.equals(item.get("organizationId")) || !runId.equals(item.get("runId"))) {
