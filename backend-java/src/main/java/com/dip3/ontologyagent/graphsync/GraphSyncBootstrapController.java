...
public final class GraphSyncBootstrapController {
    ...
    public ResponseEntity<GraphSyncRun> bootstrap(
            @RequestHeader(value = SECRET_HEADER, required = false) String secret, HttpServletRequest request) {
        auth.authenticate(secret);
        return ResponseEntity.ok(service.run(TraceFilter.from(request)));
    }
