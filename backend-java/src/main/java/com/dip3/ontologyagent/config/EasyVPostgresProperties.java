...
@ConfigurationProperties("dip3.easyv.source")
public record EasyVPostgresProperties(
  ...
  public EasyVPostgresProperties {
    ...
    if (maximumPoolSize < 1 || maximumPoolSize > 4) {
      throw new IllegalArgumentException("dip3.easyv.source.maximum-pool-size 必须在 1 到 4 之间。");
    }
    if (connectionTimeout == null || connectionTimeout.isNegative() || connectionTimeout.isZero()
      ...
      throw new IllegalArgumentException("dip3.easyv.source PostgreSQL 超时必须为正数。");
    }
    if (!schema.matches("[A-Za-z_][A-Za-z0-9_]*")) {
      throw new IllegalArgumentException("dip3.easyv.source.schema 不是合法的 PostgreSQL schema 标识符。");
    }
